...
## В чем отличия между useEffect и useLayoutEffect?

useEffect запускается асинхронно после завершения цикла рендеринга компонента и обновления экрана. Он подходит для задач, которые не требуют немедленного синхронного обновления DOM, например для вызовов API или настройки подписок

useLayoutEffect запускается синхронно сразу после всех изменений DOM, но до того, как браузер отрисует изменения. Он используется для операций, которые должны синхронизироваться с DOM, например для прямого манипулирования DOM или корректировки макета до того, как браузер отрисует его.
